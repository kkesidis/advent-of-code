...
const getInstructionModes = (instruction) => {
    ...
    return {
        instruction,
        first: parseInt(instruction.slice(2, 3)),
        second: parseInt(instruction.slice(1, 2)),
        third: parseInt(instruction.slice(0, 1)),
    }
};
...
const getNumber = (mode, index) => {
    ...
    // relative base
        ...
        return input[pointer + index] + relativeBase;
    }
}
...
const addHandler = (opCode, modes) => {
    ...
    let numberTarget = getNumber(modes.third, 3);

    input[numberTarget] = input[numberOne] + input[numberTwo];
...
const saveToPositionHandler = (opCode, modes) => {
    input[getNumber(modes.first, 1)] = systemIdInput;

    return pointer + 2;
}

const outputHandler = (opCode, modes) => {
    let output = input[getNumber(modes.first, 1)];
...
const lessThanHandler = (opCode, modes) => {
    ...
    input[numberTarget] = (input[numberOne] < input[numberTwo]) ? 1 : 0;
...
const adjustRelativeBaseHandler = (opCode, modes) => {
    let value = input[getNumber(modes.first, 1)];
    relativeBase += value;
    ...
    while (pointer < input.length && pointer != -1) {
        ...
        switch (opCode) {
            ...
            case 8:
                ...
                break;
            case 9:
                pointer = adjustRelativeBaseHandler(opCode, modes);
                break;
            case 99:
                pointer = -1;
                break;
