...
const findCrossingPoints = (wireOneCoordinates, wireTwoCoordinates) => {
  let arrayOne, arrayTwo;

  if (wireOneCoordinates.length >= wireTwoCoordinates) {
    arrayOne = wireOneCoordinates.slice();
    arrayTwo = wireTwoCoordinates.slice();
  } else {
    arrayOne = wireTwoCoordinates.slice();
    arrayTwo = wireOneCoordinates.slice();
  }

  return arrayOne.filter(coordinatesOne => arrayTwo.includes(coordinatesOne));
}

const findClosesCrossManhattanDistance = (coordinatesArray) => {
  let c, x, y, manhattanDistance = 1000;
  ...
  coordinatesArray.map(coordinates => {
    ...
    let coordinatesDistance = Math.abs(x) + Math.abs(y);

    console.log(`Coordinates: ${coordinates}, distance: ${coordinatesDistance}`);

    if (coordinatesDistance < manhattanDistance) {
      manhattanDistance = coordinatesDistance
    }
  });
...
const taskOne = () => {
  const wireOneCoordinates = drawPath(wireOnePath);
  const wireTwoCoordinates = drawPath(wireTwoPath);

  const wireCrossingCoordinates = findCrossingPoints(wireOneCoordinates, wireTwoCoordinates);

  const closestDistance = findClosesCrossManhattanDistance(wireCrossingCoordinates);
