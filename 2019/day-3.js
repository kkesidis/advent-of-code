...
let coordinates = [0, 0];
let wireOneCoordinates = [];
let wireTwoCoordinates = [];
let wireCrossingCoordinates = [];
let path = [];
let index = 0;
let direction, distance;

const resetCoordinates = () => { coordinates = [0, 0]; }
...
const drawPath = (array) => {
    ...
    array.map(movement => {
        ...
        for (x = 1; x <= distance; x++) {
            ...
            path.push(coordinates.toString());
        }
    });
...
const findClosesCrossPoint = (coordinatesArray) => {
    let closest = [1000, 1000], c, x, y;
    ...
    coordinatesArray.map(coordinates => {
        ...
        if (Math.abs(x) <= Math.abs(closest[0]) && Math.abs(y) <= Math.abs(closest[1])) {
            closest = [x, y];
        }
    });
...
const taskOne = () => {
    wireOneCoordinates = drawPath(wireOnePath);
    wireTwoCoordinates = drawPath(wireTwoPath);
    wireCrossingCoordinates = findCrossingPoints(wireOneCoordinates, wireTwoCoordinates);

    console.log('wireCrossingCoordinates', wireCrossingCoordinates.toString());

    closesCrossPoint = findClosesCrossPoint(wireCrossingCoordinates);
    console.log('closesCrossPoint', closesCrossPoint);
