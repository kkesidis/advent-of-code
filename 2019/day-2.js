 ...
 *
 * Opcode 1 adds together numbers read from two positions and stores the target in a third position. The three integers
 ...
 * emory is called an address (for example, the first value in memory is at "address 0").
 * 
...
function taskOne() {
    while (pointer <= inputLength) {
        ...
        addressOne = input[pointer + 1];
        numOne = input[addressOne];

        addressTwo = input[pointer + 2];
        numTwo = input[addressTwo];

        target = input[pointer + 3];

        if (action === 1) {
            input[target] = numOne + numTwo;
            console.log(`(${target}) => (${addressOne})${numOne} + (${addressTwo})${numTwo}`);
        } else if (action === 2) {
            input[target] = numOne * numTwo;
            console.log(`(${target}) => (${addressOne})${numOne} * (${addressTwo})${numTwo}`);
        } else if (action === 99) {
            console.log('Terminating program. pointer 0 value is: ', input[0]);
            return;
        } else {
            console.log('Wrong input detected. Expected 1,2 or 99. Got: ', action);
            return;
        }

        if (input[0] === stopAtValue) {
            console.log(`Position 0 value is ${stopAtValue}. 100 * noun * verb is: ${100 * numOne + numTwo}`);
            return;
        }
