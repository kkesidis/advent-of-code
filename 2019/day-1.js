...
let moduleFuelNeeded = 0;

const calculateFuelNeeded = mass => Math.floor(mass / 3) - 2;

moduleMasses.map(mass => {
    moduleFuelNeeded = calculateFuelNeeded(mass);

    while (moduleFuelNeeded > 0) {
        totalFuelNeeded += moduleFuelNeeded;
        moduleFuelNeeded = calculateFuelNeeded(moduleFuelNeeded);
    }
});

console.log('Total fuel needed for modules is: ', totalFuelNeeded);